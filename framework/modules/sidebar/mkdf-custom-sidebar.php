 ...
 */
if ( ! class_exists( 'WilmerMikadoClassSidebar' ) ) {
	class WilmerMikadoClassSidebar {
		var $sidebars = array();
		var $stored = "";
		var $title = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'wilmer' );
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'wilmer') . "' name='mkdf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'wilmer') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'wilmer_mikado_filter_custom_widget_args', $args );
