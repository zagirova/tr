<?php
namespace WilmerCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'wilmer' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'wilmer' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
						...
						array(
							...
							'heading'     => esc_html__( 'Product Info Type', 'wilmer' ),
							'value'       => array(
								esc_html__( 'Title', 'wilmer' )          => 'title',
								esc_html__( 'Featured Image', 'wilmer' ) => 'featured_image',
								esc_html__( 'Category', 'wilmer' )       => 'category',
								esc_html__( 'Excerpt', 'wilmer' )        => 'excerpt',
								esc_html__( 'Price', 'wilmer' )          => 'price',
								esc_html__( 'Rating', 'wilmer' )         => 'rating',
								esc_html__( 'Add To Cart', 'wilmer' )    => 'add_to_cart',
								esc_html__( 'Tag', 'wilmer' )            => 'tag',
								esc_html__( 'Author', 'wilmer' )         => 'author',
								esc_html__( 'Date', 'wilmer' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'wilmer' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'wilmer' ),
							'value'      => array(
								esc_html__( 'Default', 'wilmer' )        => '',
								esc_html__( 'Original', 'wilmer' )       => 'full',
								esc_html__( 'Square', 'wilmer' )         => 'wilmer_mikado_image_square',
								esc_html__( 'Landscape', 'wilmer' )      => 'wilmer_mikado_image_landscape',
								esc_html__( 'Portrait', 'wilmer' )       => 'wilmer_mikado_image_portrait',
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'wilmer' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'wilmer' ),
							'value'      => array(
								esc_html__( 'Default', 'wilmer' ) => '',
								esc_html__( 'White', 'wilmer' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'wilmer' ),
							'value'      => array(
								esc_html__( 'Default', 'wilmer' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'wilmer' ) ),
							'group'      => esc_html__( 'Product Info Style', 'wilmer' )
						)
					)
				)
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'wilmer' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'wilmer' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'wilmer' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'wilmer' ) . ': ' . $product_id;
