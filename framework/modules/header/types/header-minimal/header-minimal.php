...
class HeaderMinimal extends HeaderType {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			$this->menuAreaHeight     = wilmer_mikado_set_default_menu_height_for_header_types();
	...
	public function loadTemplate( $parameters = array() ) {
		...
		$parameters['menu_area_in_grid'] = wilmer_mikado_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' ? true : false;
		$parameters['fullscreen_menu_icon_class'] = wilmer_mikado_get_fullscreen_menu_icon_class();
		
		$parameters = apply_filters( 'wilmer_mikado_filter_header_minimal_parameters', $parameters );
	...
	public function calculateHeightOfTransparency() {
		...
		$menu_background_color             = wilmer_mikado_get_meta_field_intersect( 'menu_area_background_color', $id );
		$menu_background_transparency      = wilmer_mikado_get_meta_field_intersect( 'menu_area_background_transparency', $id );
		$menu_grid_background_color        = wilmer_mikado_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = wilmer_mikado_options()->getOptionValue( 'menu_area_grid_background_transparency' );
		...
		if ( $menuAreaTransparent ) {
			...
			if ( ( $sliderExists && wilmer_mikado_is_top_bar_enabled() )
			     || wilmer_mikado_is_top_bar_enabled() && wilmer_mikado_is_top_bar_transparent()
			) {
				$transparencyHeight += wilmer_mikado_get_top_bar_height();
			}
		}
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		$menu_background_color             = wilmer_mikado_options()->getOptionValue( 'menu_area_background_color' );
		$menu_background_transparency      = wilmer_mikado_options()->getOptionValue( 'menu_area_background_transparency' );
		$menu_grid_background_color        = wilmer_mikado_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = wilmer_mikado_options()->getOptionValue( 'menu_area_grid_background_transparency' );
	...
	public function calculateHeaderHeight() {
		...
		if ( wilmer_mikado_is_top_bar_enabled() ) {
			$headerHeight += wilmer_mikado_get_top_bar_height();
		}
	...
	public function getPerPageJSVariables( $perPageVars ) {
		...
		$header_behavior = wilmer_mikado_get_meta_field_intersect( 'header_behaviour' );
		if ( ! in_array( $header_behavior, array( 'sticky-header-on-scroll-up', 'sticky-header-on-scroll-down-up' ) ) ) {
			$perPageVars['mkdfHeaderTransparencyHeight'] = $this->headerHeight - ( wilmer_mikado_get_top_bar_height() + $this->heightOfCompleteTransparency );
		} else {
